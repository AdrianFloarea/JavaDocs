...
public class Exercise1 {
    ...
    public List<String> iteratingOverKeys(){
        ...
        for(String s:countries.keySet()){
            if(s!=null){
                if(s.charAt(0)=='R'){seekingCountries.add(s);}
            }
        }
    ...
    public String iteratingOverValues(){
        ...
        seekingCapital=countries.get(new ArrayList<String>(countries.keySet()).get(0));
        int maxLength=seekingCapital.length();
        for(String s:countries.keySet()){
            String capital=countries.get(s);
